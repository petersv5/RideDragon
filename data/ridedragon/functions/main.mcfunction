...
execute as @p[nbt={RootVehicle:{Entity:{Tags:["dragonseat"]}}},nbt={SelectedItem:{tag:{DragonBook:1}}}] at @s positioned ^ ^ ^1.0 run execute positioned ~ ~-0.85 ~ if block ~ ~-1.3 ~ air run tp @e[tag=dragonfacing,sort=nearest,distance=..3,limit=1] ~ ~ ~ ~ ~
...
execute as @e[tag=dragonseat] at @s if entity @p[distance=..3,nbt={RootVehicle:{Entity:{Tags:["dragonseat"]}},SelectedItem:{tag:{DragonBook:1}}}] positioned ^ ^ ^-2.7 run execute rotated as @s run tp @e[tag=dragonvisible,limit=1,sort=nearest,distance=..4] ~ ~-0.7 ~ ~-180 ~

#execute as @e[tag=dragonseat] at @s if entity @p[distance=..3,nbt={RootVehicle:{Entity:{Tags:["dragonseat"]}},SelectedItem:{tag:{DragonBook:1}}}] positioned ^ ^ ^-0.6 run execute rotated ~ 0 positioned ^ ^ ^-2.0 run execute rotated as @s run tp @e[tag=dragonvisible,limit=1,sort=nearest,distance=..4] ~ ~-0.7 ~ ~-180 ~
